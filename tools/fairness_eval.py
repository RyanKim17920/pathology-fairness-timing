...
# <repo>/<cohort>/* via snapshot_download). TCGA in-distribution tasks
# (nsclc/glioma/brca) read local /data/TCGA-12K-parquet and need NO HF pull, so
# they map to None; only the external CPTAC task is fetched from HF.
TASK_TO_COHORT = {
    "nsclc": None,                # TCGA-12K (local /data/TCGA-12K-parquet)
    "glioma": None,               # TCGA-12K (local /data/TCGA-12K-parquet)
    "brca": None,                 # TCGA-12K (local /data/TCGA-12K-parquet)
...
def maybe_pull_hf_tiles(task, tiles_dir, hf_repo, out_path, log=print):
    ...
    if cohort is None:
        raise SystemExit(
            ...
            f"(TCGA in-distribution tasks read local /data/TCGA-12K-parquet); "
...
def main():
    ...
    ap.add_argument("--hf-repo", default=None,
                    help="HF dataset repo id (e.g. ryankim17920/nanopath-fairness-tiles); "
    ...
    model, mean, std, binfo = build_backbone(args.checkpoint, device, args.variant)
