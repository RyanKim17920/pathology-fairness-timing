#!/bin/bash
# driver_cc.sh: A_cancercond arm x 3 folds (3 runs)
PYTHON="/admin/home/ryan.kim/nanopath/nanopath-tests/20260710_fino-fairness/.venv/bin/python"
CKPT="/data/ryan.kim/nanopath/vendor-contrastive-cancercond-siteholdout/latest.pt"
...
run_one() {
  ...
  $PYTHON tools/post_hoc_debias.py \
    ...
    --method contrastive \
    --lambda-adv 0.0 \
    --dump-predictions "/data/ryan.kim/nanopath/results/preds/hh_A_cancercond__brca_tp53__${FOLD}.jsonl" \
    --out "/data/ryan.kim/nanopath/results/hh_A_cancercond__${FOLD}.json" \
    || echo "FAILED A_cancercond ${FOLD}"
}
...
echo "========== $(date): driver_cc.sh COMPLETE =========="