...
The runner is path-agnostic: local data, metadata, cache and output locations
are supplied explicitly on the command line.
"""
import argparse
import glob
import hashlib
...
def build_head(in_dim, hidden, sensitive, dropout=0.1, method="dann",
    ...
    separate prototype bank for each conditioning stratum. ``n_conditions`` may
    be larger than two when conditioning on cancer type. The original marginal
    modules remain available for tiles whose condition is unknown."""
    torch = _torch()
    ...
    class DebiasHead(nn.Module):
                ...
                for a in attrs:
                    ...
                    if method == "fino" and self.condition_on_label:
                        self.register_buffer(f"proto_by_y_{a}",
                                             torch.zeros(self.n_conditions, n, hidden))
                        self.register_buffer(
                            f"proto_by_y_init_{a}",
                            torch.zeros(self.n_conditions, n, dtype=torch.bool))
        ...
        def _conditional_logits(self, heads, h, task_labels,
            ...
            yidx = task_labels.clamp(min=0, max=self.n_conditions - 1)
            out = {}
            for a in self.attrs:
                by_y = torch.stack(
                    [heads[a][y](h) for y in range(self.n_conditions)], dim=1)
                ...
                for a in self.attrs:
                    if self.condition_on_label and task_labels is not None:
                        proto_by_y = F.normalize(
                            ...
                            dim=2, eps=1e-6)
                        selected = proto_by_y[
                            task_labels.clamp(min=0, max=self.n_conditions - 1)]
                        logits = (
...
def _fair_supcon(h, labels_dict, attrs, temp, torch, F, weights=None,
    ...
    for a in attrs:
        ...
        if task_labels is not None:
            known_condition = task_labels >= 0
            pos = (pos
                   & known_condition.unsqueeze(0)
                   & known_condition.unsqueeze(1)
                   & (task_labels.unsqueeze(0) == task_labels.unsqueeze(1)))
        pos_cnt = pos.sum(1)
...
def _train_eval_projfree(emb_task, bc_task, emb_pool, bc_pool, label_of, fold_of,
    ...
    condition_task = np.asarray([
        (condition_of or label_of).get(bc, -1) for bc in bc_task
    ], dtype=np.int64)
    condition_pool = np.asarray([
        (condition_of or {}).get(bc, -1) for bc in bc_pool
    ], dtype=np.int64)
    ...
    ytr_long = torch.tensor(condition_task[is_train_task],
    ...
    aidx_pool_t = ({a: torch.tensor(aidx_pool[a], device=device) for a in attrs}
                   if has_pool else None)
    condition_pool_t = (torch.tensor(condition_pool, device=device)
                        if has_pool and condition_on_label else None)

    n_task = Xtr.shape[0]
    n_adv = n_task + (Xpool.shape[0] if has_pool else 0)

    model = build_head(D, hidden, sensitive, method=method,
                       condition_on_label=condition_on_label,
...
def train_and_eval(emb_task, bc_task, emb_pool, bc_pool, label_of, fold_of,
    ...
    if method in ("contrastive", "pcgrad"):        # trunk-level methods (no GRL)
        return _train_eval_projfree(
            ...
            condition_of=condition_of, n_conditions=n_conditions,
            log=log)
    ...
    y_adv = (np.concatenate([
        condition_task[is_train_task], condition_pool])
        if condition_on_label else None)
    ...
    with torch.no_grad():
        eval_task_labels = (
            torch.tensor(condition_task[is_eval], dtype=torch.long, device=device)
            if condition_on_label else None)
        logit, adv_logits = model(
...
def main():
    ...
    ap.add_argument("--condition-col", default=None,
                    help="condition demographic removal on this categorical "
                         "metadata column instead of the downstream label; use "
                         "the same semantic factor as pretraining (for example, "
                         "cancer_type). Values may have any number of classes.")
    ...
    if args.condition_col:
        metadata_rows = {}
        molecular_rows = locals().get("mol", {})
        for patient_id in set(demo) | set(molecular_rows):
            metadata_rows[patient_id] = {
                **demo.get(patient_id, {}),
                **molecular_rows.get(patient_id, {}),
            }
        condition_categories = sorted({
            str(row.get(args.condition_col, "")).strip()
            for row in metadata_rows.values()
            if str(row.get(args.condition_col, "")).strip()
        })
        if len(condition_categories) < 2:
            ap.error(f"--condition-col {args.condition_col!r} has fewer than two values")
        category_id = {value: index for index, value in enumerate(condition_categories)}
        condition_of = {
            patient_id: category_id[value]
            for patient_id, row in metadata_rows.items()
            if (value := str(row.get(args.condition_col, "")).strip()) in category_id
        }
        n_conditions = len(condition_categories)
        args.condition_on_label = True
        log(f"[debias] conditioning on {args.condition_col}: "
            f"{n_conditions} classes, {len(condition_of)} patients")
    tile_cohort = cohort
    ...
    if args.hospital_fold:
        import csv as _csv
        hf_csv = args.hospital_folds_csv
        if not hf_csv:
            ap.error("--hospital-fold requires --hospital-folds-csv")
    ...
    for lam in lambdas:
        ...
        runs[tag] = train_and_eval(
            ...
            condition_of=condition_of, n_conditions=n_conditions, log=log)
